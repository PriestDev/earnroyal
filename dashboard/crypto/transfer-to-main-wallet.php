...
<head>
    ...
    <title> Bluewave ISO - Transfer to Main Wallet</title>
    <meta name="title" Content="Bluewave ISO - Transfer to Main Wallet">
    ...
    <meta name="apple-mobile-web-app-title" content="Bluewave ISO - Transfer to Main Wallet">
    
    <meta itemprop="name" content="Bluewave ISO - Transfer to Main Wallet">
    ...
    <meta property="og:url" content="https://bluewave-iso.net/user/crypto/transfer">
...
<div class="d-flex flex-wrap">
    ...
    <li>
        ...
        <ul class="sidebar-submenu">
            ...
            <li><a href="https://bluewave-iso.net/user/crypto/transfer" class="active">Transfer to Wallet</a></li>
...
body.inverted video {
  filter: invert(1) hue-rotate(180deg);
    ...
    <div class="mb-4">
        <h3 class="text-secondary">Transfer to Main Wallet</h3>
        <p class="text-muted">Convert your cryptocurrency to USD and transfer to your main wallet</p>
    </div>
    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body">
                    <form action="https://bluewave-iso.net/user/crypto/transfer" method="POST" id="transferForm">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="coin_id" id="coinSelect" required>
                                <option value="">Choose cryptocurrency to transfer</option>
                                                            </select>
                                                            <small class="text-muted text-danger">No cryptocurrency balance available for transfer</small>
                                                    </div>

                        <div class="form-group">
                            <label class="form-label">Amount to Transfer</label>
                            <div class="input-group">
                                <input type="number" step="0.00000001" class="form-control" name="amount" id="cryptoAmount" placeholder="Select a coin first..." disabled required>
                                <span class="input-group-text" id="cryptoSymbol">-</span>
                            </div>
                        ...
                        <!-- Transfer Details -->
                        <div class="card bg-light mt-3" id="transferDetails" style="display: none;">
                            <div class="card-body">
                                <h6 class="card-title">Transfer Summary</h6>
                                <div class="row">
                                    <div class="col-md-6">
                                        <small class="text-muted">Current Price</small>
                                        <p class="mb-2" id="currentPrice">$0.00</p>
                                    </div>
                                    <div class="col-md-6">
                                        <small class="text-muted">Transfer Fee</small>
                                        <p class="mb-2" id="transferFee">$0.00</p>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-6">
                                        <small class="text-muted">Gross USD Value</small>
                                        <p class="mb-2" id="grossValue">$0.00</p>
                                    </div>
                                    <div class="col-md-6">
                                        <small class="text-muted">Net USD Amount</small>
                                        <p class="mb-2 text--primary" id="netAmount">$0.00</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="alert alert-info mt-3">
                            ...
                            Your cryptocurrency will be converted to USD at the current market price and transferred to your main wallet after deducting the transfer fee.                        </div>
...
(function ($) {
    ...
    function bindEvents() {
        ...
        $('#cryptoAmount, input[name="amount"]').off('input').on('input', calculateTransfer);
        
        // Bind max button
        $('#maxButton').off('click').on('click', function() {
            let amountInput = $('#cryptoAmount');
        ...
        $('.select-crypto-btn').off('click').on('click', function() {
            ...
            $('#coinSelect').val(coinId).trigger('change');
    ...
    function handleCoinSelection() {
        ...
        transferFee = parseFloat(selected.data('fee')) || 0;
        
        // Update UI elements
        $('#cryptoSymbol').text(currentSymbol.toUpperCase());
        $('#availableBalance').text(availableBalance.toFixed(8));
        $('#availableSymbol').text(currentSymbol.toUpperCase());
    ...
    function calculateTransfer() {
        let cryptoAmountInput = $('#cryptoAmount').val();
        if (!cryptoAmountInput) {
            cryptoAmountInput = $('input[name="amount"]').val();
        }
        let cryptoAmount = parseFloat(cryptoAmountInput) || 0;
        
        if (cryptoAmount <= 0 || currentPrice <= 0) {
            $('#transferDetails').hide();
            return;
        }
        
        let grossUsdValue = cryptoAmount * currentPrice;
        let feeAmount = (grossUsdValue * transferFee) / 100;
        let netUsdValue = grossUsdValue - feeAmount;
        
        $('#grossValue').text('$' + grossUsdValue.toFixed(2));
        $('#transferFee').text('$' + feeAmount.toFixed(2) + ' (' + transferFee + '%)');
        $('#netAmount').text('$' + netUsdValue.toFixed(2));
    ...
    function handleFormSubmit(e) {
        ...
        let cryptoAmount = parseFloat(cryptoAmountInput);
        
        // Check if coin is selected
        if (!selectedCoin) {
            e.preventDefault();
            alert('Please select a cryptocurrency to transfer');
            $('#coinSelect, select[name="coin_id"]').focus();
            return false;
        }
        
        // Check if amount is entered
        if (!cryptoAmountInput || cryptoAmountInput.trim() === '') {
            e.preventDefault();
        ...
        // Final confirmation
        let netValue = ((cryptoAmount * currentPrice) * (100 - transferFee)) / 100;
        let confirmMessage = 'Are you sure you want to transfer ' + cryptoAmount.toFixed(8) + ' ' + currentSymbol.toUpperCase() + ' to your main wallet? You will receive $' + netValue.toFixed(2) + ' after fees.';
